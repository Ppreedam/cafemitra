...
2. `cafemitra_server` API ko poll karke print jobs uthaaye, print kare, aur status wapas server ko report kare.
...
**Scope me NAHI hai** (abhi ke liye): macOS/Linux support, multi-shop/multi-account ek hi instance me, cloud-hosted print queue (already cafemitra_server handle karta hai), passport AI photo / Gemini automation (`agent/passport-jobs/` queue) — filhaal purane C# agent se hi chalta rahega, is rewrite me carry nahi kiya jaa raha.
...
├── models.py                  # PrintJob, AuthResponse, DTOs (C#: Models.cs)
├── printing/
...
### 4.4 Local Bridge Server (127.0.0.1:8765)
...
### 4.6 UI / UX
- [ ] Navigation: Account, Print, History, Settings tabs
...
3. **Auto-update mechanism ek hi jagah se** — decide karo GitHub raw-files ya `cafemitra_server`'s `/api/agent/version-check/`+`/update/` (abhi dono maujood hain but dusra istemal nahi hota, confusing hai). §6 me decide karo.
4. **Token storage encrypt karo** — abhi C# me `AccessToken`/`RefreshToken` plain-text `config.json` me hain jabki password DPAPI-encrypted hai. Python rewrite me tokens bhi `win32crypt` se encrypt karo (same protection level jo password ko milta hai).
5. **Local bridge ko halka sa auth do** — poori tarah open mat rakho; ek simple shared-secret (agent-generated, website ko config me dikhaya jaaye) add karne se accidental cross-app access rukega, without breaking the localhost-only trust model.
6. **`ColorMode` field**: agar server-side kabhi add ho (dekho `PRINT_AGENT_API.md` §3), to client isse turant respect kare — abhi ke text-guessing fallback ko bhi rakho as backward-compat.
...
### Phase 4 — QR Print, Printer Presets
...
### Phase 7 — Hardening & Edge-Case Testing
