...
// The "Customers" section and the "Shops" section are two different admin
// views over the exact same underlying accounts, so this page reuses every
// shop-detail endpoint/action (they already key off the raw user id) and
// only adds customer-specific extras (last seen, coupons used) on top.
export default function CustomerDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [customerExtra, setCustomerExtra] = useState<{ customer: Customer; orderCount: number; couponsUsed: CustomerCouponUsed[] } | null>(null);
  ...
  useEffect(() => {
    ...
    fetchShopOrders(customerId, { status: orderStatusFilter, from: orderFrom, to: orderTo, page: orderPage })
      .then((res) => {
  ...
  }, [tab, customerId, orderStatusFilter, orderFrom, orderTo, orderPage]);
  ...
  }, [tab, customerId, txnPage]);

  function load() {
    fetchShopDetail(customerId)
      .then((res) => {
        setData(res);
    ...
    fetchCustomerDetail(customerId)
      .then(setCustomerExtra)
      .catch(() => {
        // Last-seen/coupons are a convenience add-on on top of the shop
        // record - a failed fetch shouldn't block the rest of the page.
      });
    fetchActivityLog({ targetType: "shop", targetId: String(customerId) })
  ...
  async function handleToggleActive() {
    if (!data) return;
    try {
      await setShopActive(customerId, !data.shop.isActive);
      load();
    } catch (err) {
  ...
  async function handleSendPasswordReset() {
    setActionMessage("");
    try {
      const res = await sendShopPasswordReset(customerId);
  ...
  async function handleImpersonate() {
    if (!window.confirm("This opens the cafe-owner dashboard as this customer, in a new tab. Logged in the activity log. Continue?")) {
      return;
    }
    try {
      await impersonateShop(customerId);
    } catch (err) {
  ...
  const couponsUsed = customerExtra?.couponsUsed || [];
  ...
  return (
    <div>
      ...
      <div className="flex items-start justify-between mb-4 flex-wrap gap-2">
        <div>
          <h1 className="text-xl font-semibold text-slate-900">{data.shop.fullName || "(unnamed)"}</h1>
          <p className="text-sm text-slate-500 flex flex-wrap items-center gap-1.5">
            <span className="flex items-center gap-1">
              {data.shop.email}
              {data.shop.email && <CopyButton value={data.shop.email} />}
            </span>
            ·
            <span className="flex items-center gap-1">
              {data.shop.phone}
              {data.shop.phone && <CopyButton value={data.shop.phone} />}
            </span>
          </p>
        </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4 mb-4">
        <dl className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
          <div>
            ...
            <dd className={`font-medium ${data.shop.balance < 0 ? "text-red-600" : "text-slate-900"}`}>{formatCurrency(data.shop.balance)}</dd>
          </div>
          <div>
          ...
          <div>
            ...
            <dd className="text-slate-900">{new Date(data.shop.dateJoined).toLocaleDateString("en-IN")}</dd>
          </div>
          <div>
            <dt className="text-slate-500">Last seen</dt>
            <dd className="text-slate-900">
              {customerExtra?.customer.lastSeen ? new Date(customerExtra.customer.lastSeen).toLocaleDateString("en-IN") : "-"}
            </dd>
          </div>
          <div>
            <dt className="text-slate-500">Orders generated so far</dt>
            <dd className="text-slate-900 font-medium">{customerExtra?.orderCount ?? "-"}</dd>
          </div>
        </dl>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4 max-w-xl">
          ...
          <div className="border-t border-slate-100 pt-4">
            ...
            <label className="flex items-center gap-2 text-sm text-slate-700 mb-4">
              <input type="checkbox" checked={cashCounterInput} onChange={(e) => setCashCounterInput(e.target.checked)} />
      ...
      {showDeleteModal && (
        <DeleteShopModal
          shopId={customerId}
          shopName={data.shop.shopName || data.shop.fullName}
