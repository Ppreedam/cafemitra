...
const ACCEPT = "image/png,image/webp,image/gif,image/bmp,image/svg+xml,.png,.webp,.gif,.bmp,.svg,.heic,.heif";

export default function ConvertToJpgClient({ children }: { children?: ReactNode }) {
  ...
  async function addFiles(files: FileList | File[]) {
    ...
    if (invalid) return setError(`${invalid.name} is not a supported PNG, WebP, GIF, BMP, SVG, or HEIC image.`);
    setError("");
    const additions: ImageItem[] = await Promise.all(selected.map(async (file) => {
      ...
      if (isHeic(file)) {
        try {
          ...
          // Leave the original object URL - imageDimensions will fail to decode it and surface the standard "could not be opened" error below.
        }
      }
      return { id: crypto.randomUUID(), file, preview, width: 0, height: 0, loading: true };
    }));
  ...
  return <DashboardShell activePath="/image-tools">
    <div className="dashboard compress-pdf-page">
      ...
      {!items.length ? <><PdfToolUpload title="Convert to JPG" description="Convert PNG, WebP, GIF, BMP, SVG, or HEIC images into JPG - one at a time or in a batch." icon={FileImage} inputRef={inputRef} onFiles={(files) => void addFiles(files)} accept={ACCEPT} buttonLabel="Select images" dropLabel="or drop PNG, WebP, GIF, BMP, SVG, or HEIC images here" headingLevel={children ? "h2" : "h1"} backHref="/image-tools" backLabel="Image Tools" />{children}</> : <div className="compress-studio">
        ...
        <aside className="compress-side-panel"><div><span className="auto-print-kicker">RepetiGo Image Tools</span><h2>Convert to JPG</h2><p>{items.length} files · {completed} converted</p></div>
...
function ImagePreviewModal({ item, onClose }: { item: ImageItem; onClose: () => void }) {
  ...
  return <div className="compress-preview-backdrop" role="dialog" aria-modal="true" aria-label={`Preview for ${item.file.name}`} onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className="compress-preview-modal image-compress-preview"><header><div><h2>Convert to JPG preview</h2><p>{item.file.name} · {item.width} × {item.height}px</p></div><button type="button" onClick={onClose} aria-label="Close preview"><X size={21} /></button></header><div className="image-compress-compare"><article><div><strong>Original file</strong><span>{formatBytes(item.file.size)}</span></div><img className="compress-thumbnail-transparent" src={item.preview} alt="Original image" /></article><article><div><strong>Converted JPG</strong><span>{formatBytes(item.result.size)} · {savingText(item.file.size, item.result.size)}</span></div><img src={item.result.url} alt="Converted JPG" /></article></div><footer><div><span>Transparent areas are filled with white</span></div><a href={item.result.url} download={item.result.name}><Download size={18} /> Download JPG</a></footer></section></div>;
}

function isHeic(file: File) { return file.type === "image/heic" || file.type === "image/heif" || /\.(heic|heif)$/i.test(file.name); }
function isSupported(file: File) { return ["image/png", "image/webp", "image/gif", "image/bmp", "image/svg+xml"].includes(file.type) || /\.(png|webp|gif|bmp|svg|heic|heif)$/i.test(file.name); }
