...
export default function RotateImageClient({ children }: { children?: ReactNode }) {
  ...
  async function downloadZip() {
    ...
    try { const zip = new JSZip(); completed.forEach((item) => zip.file(item.result!.name, item.result!.blob)); const blob = await zip.generateAsync({ type: "blob", compression: "DEFLATE", compressionOptions: { level: 6 } }); triggerDownload(URL.createObjectURL(blob), "repetigo-rotated-images.zip", true); }
