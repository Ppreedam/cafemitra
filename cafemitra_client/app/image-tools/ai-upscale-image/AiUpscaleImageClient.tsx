...
import { Download, LoaderCircle, Plus, ShieldCheck, Sparkles, Trash2, WandSparkles } from "lucide-react";
...
export default function AiUpscaleImageClient({ children }: { children?: ReactNode }) {
  ...
  return <DashboardShell activePath="/image-tools"><div className="dashboard upscale-page">
    ...
    {items.length ? <div className="compress-heading upscale-heading"><div><span className="auto-print-kicker">AI Image Tool</span><h2>AI Image Upscaler</h2><p>Enlarge photos and scans with AI-added detail, not just stretched pixels.</p></div><span><ShieldCheck size={16} /> Auto-deleted from our servers</span></div> : null}
    {!items.length ? <PdfToolUpload title="AI Image Upscaler" description="Enlarge blurry scans and low-resolution photos with AI-added detail." icon={Sparkles} inputRef={inputRef} onFiles={(files) => void addFiles(files)} accept="image/jpeg,image/png,image/webp,.jpg,.jpeg,.png,.webp" buttonLabel="Select images" dropLabel="or drop JPG, PNG, and WebP images here" headingLevel={children ? "h2" : "h1"} /> : <section className={`upscale-studio ${dragging ? "dragging" : ""}`} onDragOver={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={drop}>
      <button className={`upscale-ai-suggestion ${aiMode ? "active" : ""}`} type="button" aria-pressed={aiMode} onClick={() => setAiMode((current) => !current)}><Sparkles size={15} /><div><strong>{aiMode ? "AI enhancement enabled" : "Standard upscaler enabled"}</strong><span>{aiMode ? "Switch to the free standard upscaler" : "Switch back to AI Upscale"}</span></div></button>
