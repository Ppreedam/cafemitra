...
type OutputFormat = "jpg" | "webp" | "gif" | "bmp" | "ico" | "pdf";
...
export default function PngConverterClient({ children }: { children?: ReactNode }) {
  ...
  async function convertFiles(targets: ImageItem[], formatValue: OutputFormat, qualityValue: number) {
    ...
    try {
      for (const item of targets) {
        discard(item);
        try {
          const output = await convertPng(item.preview, item.width, item.height, formatValue, qualityValue);
  ...
  return <DashboardShell activePath="/image-tools">
    <div className="dashboard compress-pdf-page">
      {items.length ? <><input ref={inputRef} hidden multiple type="file" accept="image/png,.png" onChange={(event: ChangeEvent<HTMLInputElement>) => { if (event.target.files) void addFiles(event.target.files); event.target.value = ""; }} /><div className="compress-heading"><div><span className="auto-print-kicker">Free Image Tool</span><h2>PNG Converter</h2><p>Convert PNG images into JPG, WebP, GIF, BMP, ICO, or PDF.</p></div><span><ShieldCheck size={16} /> Files stay in your browser</span></div></> : null}
      {!items.length ? <><PdfToolUpload title="PNG Converter" description="Convert PNG images into JPG, WebP, GIF, BMP, ICO, or PDF - one at a time or in a batch." icon={FileImage} inputRef={inputRef} onFiles={(files) => void addFiles(files)} accept="image/png,.png" buttonLabel="Select PNG images" dropLabel="or drop PNG images here" headingLevel={children ? "h2" : "h1"} backHref="/image-tools" backLabel="Image Tools" />{children}</> : <div className="compress-studio">
        <section className={`compress-workspace ${dragging ? "dragging" : ""}`} onDragOver={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={drop}>
          <div className="compress-workspace-head"><div><h2>Your PNG files</h2><p>Files convert automatically after upload.</p></div><button type="button" disabled={converting} onClick={() => inputRef.current?.click()}><Plus size={18} /> Add images</button></div>
        ...
        <aside className="compress-side-panel"><div><span className="auto-print-kicker">RepetiGo Image Tools</span><h2>PNG Converter</h2><p>{items.length} files · {completed} converted</p></div>
          <div className="compress-side-config">
            <div className="compress-level-label"><Info size={20} /><div><strong>Output format</strong><small>Choose the file type you need PNG converted to.</small></div></div>
...
function formatNote(format: OutputFormat) {
  ...
  if (format === "ico") return "ICO output is best suited to small square icons and favicons.";
  if (format === "pdf") return "The PNG is embedded as a single full-page image in the PDF.";
  if (format === "bmp") return "BMP is uncompressed - files are larger than PNG, JPG, or WebP.";
  return "At quality 90+, output is visually identical to the PNG source.";
}
...
async function convertPng(url: string, width: number, height: number, format: OutputFormat, quality: number) {
  const image = await loadImageElement(url);
  const canvas = document.createElement("canvas"); canvas.width = width; canvas.height = height;
  const context = canvas.getContext("2d"); if (!context) throw new Error("Canvas unavailable");
  if (format === "jpg" || format === "bmp") { context.fillStyle = "#fff"; context.fillRect(0, 0, width, height); }
