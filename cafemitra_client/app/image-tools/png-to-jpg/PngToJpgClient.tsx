...
export default function PngToJpgClient({ children }: { children?: ReactNode }) {
  ...
  async function addFiles(files: FileList | File[]) {
    ...
    if (invalid) return setError(`${invalid.name} is not a supported PNG, WebP, GIF, or SVG image.`);
    setError("");
  ...
  async function convertFiles(targets: ImageItem[], qualityValue: number) {
    ...
    try {
      for (const item of targets) {
        discard(item);
        const output = await convertToJpg(item.file, item.preview, item.width, item.height, qualityValue);
  ...
  async function downloadZip() {
    ...
    try { const zip = new JSZip(); completed.forEach((item) => zip.file(item.result!.name, item.result!.blob)); const blob = await zip.generateAsync({ type: "blob", compression: "DEFLATE", compressionOptions: { level: 6 } }); triggerDownload(URL.createObjectURL(blob), "repetigo-converted-jpgs.zip", true); }
  ...
  return <DashboardShell activePath="/image-tools">
    <div className="dashboard compress-pdf-page">
      ...
      {!items.length ? <><PdfToolUpload title="PNG to JPG" description="Convert PNG, WebP, GIF, or SVG images into JPG - one at a time or in a batch." icon={ImageIcon} inputRef={inputRef} onFiles={(files) => void addFiles(files)} accept="image/png,image/webp,image/gif,image/svg+xml,.png,.webp,.gif,.svg,.heic,.heif" buttonLabel="Select images" dropLabel="or drop PNG, WebP, GIF, or SVG images here" headingLevel={children ? "h2" : "h1"} />{children}</> : <div className="compress-studio">
        <section className={`compress-workspace ${dragging ? "dragging" : ""}`} onDragOver={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={drop}>
          ...
          </article>)}<button className="compress-add-card" type="button" disabled={converting} onClick={() => inputRef.current?.click()}><span><Plus size={29} /></span><strong>Add image files</strong><small>Click or drop more files</small></button></div>
        </section>
        <aside className="compress-side-panel"><div><span className="auto-print-kicker">RepetiGo Image Tools</span><h2>PNG to JPG</h2><p>{items.length} files · {completed} converted</p></div>
...
function ImagePreviewModal({ item, onClose }: { item: ImageItem; onClose: () => void }) {
  ...
  return <div className="compress-preview-backdrop" role="dialog" aria-modal="true" aria-label={`Preview for ${item.file.name}`} onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className="compress-preview-modal image-compress-preview"><header><div><h2>PNG to JPG preview</h2><p>{item.file.name} · {item.width} × {item.height}px</p></div><button type="button" onClick={onClose} aria-label="Close preview"><X size={21} /></button></header><div className="image-compress-compare"><article><div><strong>Original PNG</strong><span>{formatBytes(item.file.size)}</span></div><img src={item.preview} alt="Original image" /></article><article><div><strong>Converted JPG</strong><span>{formatBytes(item.result.size)} · {savingText(item.file.size, item.result.size)}</span></div><img src={item.result.url} alt="Converted JPG" /></article></div><footer><div><span>Transparent areas are filled with white</span></div><a href={item.result.url} download={item.result.name}><Download size={18} /> Download JPG</a></footer></section></div>;
}

function isSupported(file: File) { return ["image/png", "image/webp", "image/gif", "image/svg+xml"].includes(file.type) || /\.(png|webp|gif|svg|heic|heif)$/i.test(file.name); }
...
async function convertToJpg(file: File, url: string, width: number, height: number, quality: number) {
