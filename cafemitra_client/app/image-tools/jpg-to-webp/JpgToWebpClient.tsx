...
export default function JpgToWebpClient({ children }: { children?: ReactNode }) {
  ...
  async function addFiles(files: FileList | File[]) {
    ...
    if (invalid) return setError(`${invalid.name} is not a supported JPG or JPEG image.`);
    setError("");
  ...
  async function convertFiles(targets: ImageItem[], qualityValue: number) {
    ...
    try {
      for (const item of targets) {
        discard(item);
        const output = await convertToWebp(item.preview, item.width, item.height, qualityValue);
        const url = URL.createObjectURL(output.blob); resultUrls.current.add(url);
        const result = { ...output, url, size: output.blob.size, name: webpName(item.file.name) };
  ...
  async function downloadZip() {
    ...
    try { const { default: JSZip } = await import("jszip"); const zip = new JSZip(); completed.forEach((item) => zip.file(item.result!.name, item.result!.blob)); const blob = await zip.generateAsync({ type: "blob", compression: "DEFLATE", compressionOptions: { level: 6 } }); triggerDownload(URL.createObjectURL(blob), "repetigo-converted-webp.zip", true); }
  ...
  return <DashboardShell activePath="/image-tools">
    <div className="dashboard compress-pdf-page">
      {items.length ? <><input ref={inputRef} hidden multiple type="file" accept="image/jpeg,.jpg,.jpeg" onChange={(event: ChangeEvent<HTMLInputElement>) => { if (event.target.files) void addFiles(event.target.files); event.target.value = ""; }} /><div className="compress-heading"><div><span className="auto-print-kicker">Free Image Tool</span><h2>JPG to WebP</h2><p>Convert JPG images into smaller WebP files.</p></div><span><ShieldCheck size={16} /> Files stay in your browser</span></div></> : null}
      {!items.length ? <><PdfToolUpload title="JPG to WebP" description="Convert JPG or JPEG images into WebP - smaller files for faster websites." icon={FileImage} inputRef={inputRef} onFiles={(files) => void addFiles(files)} accept="image/jpeg,.jpg,.jpeg" buttonLabel="Select JPG images" dropLabel="or drop JPG images here" headingLevel={children ? "h2" : "h1"} backHref="/image-tools" backLabel="Image Tools" />{children}</> : <div className="compress-studio">
        <section className={`compress-workspace ${dragging ? "dragging" : ""}`} onDragOver={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={drop}>
          <div className="compress-workspace-head"><div><h2>Your JPG files</h2><p>Files convert to WebP automatically after upload.</p></div><button type="button" disabled={converting} onClick={() => inputRef.current?.click()}><Plus size={18} /> Add images</button></div>
        ...
        <aside className="compress-side-panel"><div><span className="auto-print-kicker">RepetiGo Image Tools</span><h2>JPG to WebP</h2><p>{items.length} files · {completed} converted</p></div>
          <div className="compress-side-config"><div className="compress-level-label"><Gauge size={20} /><div><strong>WebP quality</strong><small>Higher keeps more detail; lower makes a smaller file.</small></div><span title="Images are converted locally in your browser."><Info size={17} /></span></div><input type="range" min="40" max="100" step="5" value={quality} disabled={converting} onChange={(event) => updateQuality(Number(event.target.value))} aria-label="WebP quality" /><output>{quality}<small>%</small></output></div>
          <div className="compress-side-summary"><span><Check size={18} /></span><div><strong>{converting ? "Converting files…" : completed === items.length ? "All files ready" : "Preparing conversion"}</strong><small>{converting ? "Please keep this page open." : "At quality 80+, WebP output is visually identical to the JPG source."}</small></div></div>
...
function ImagePreviewModal({ item, onClose }: { item: ImageItem; onClose: () => void }) {
  ...
  return <div className="compress-preview-backdrop" role="dialog" aria-modal="true" aria-label={`Preview for ${item.file.name}`} onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className="compress-preview-modal image-compress-preview"><header><div><h2>JPG to WebP preview</h2><p>{item.file.name} · {item.width} × {item.height}px</p></div><button type="button" onClick={onClose} aria-label="Close preview"><X size={21} /></button></header><div className="image-compress-compare"><article><div><strong>Original JPG</strong><span>{formatBytes(item.file.size)}</span></div><img src={item.preview} alt="Original image" /></article><article><div><strong>Converted WebP</strong><span>{formatBytes(item.result.size)} · {savingText(item.file.size, item.result.size)}</span></div><img src={item.result.url} alt="Converted WebP" /></article></div><footer><div><span>Smaller file, same visual quality</span></div><a href={item.result.url} download={item.result.name}><Download size={18} /> Download WebP</a></footer></section></div>;
}

function isSupported(file: File) { return file.type === "image/jpeg" || /\.(jpg|jpeg)$/i.test(file.name); }
...
async function convertToWebp(url: string, width: number, height: number, quality: number) {
  ...
  const blob = await canvasBlob(canvas, "image/webp", quality / 100);
...
function webpName(name: string) { return `${name.replace(/\.[^.]+$/, "")}.webp`; }
