...
type OutputFormat = "jpg" | "png" | "webp" | "bmp" | "ico" | "pdf" | "svg";
...
const QUALITY_FORMATS: OutputFormat[] = ["jpg", "webp"];
const WHITE_BACKGROUND_FORMATS: OutputFormat[] = ["jpg", "bmp"];
const MAX_TRACE_DIMENSION = 1000;

export default function GifConverterClient({ children }: { children?: ReactNode }) {
  ...
  async function addFiles(files: FileList | File[]) {
    ...
    if (invalid) return setError(`${invalid.name} is not a supported GIF image.`);
    setError("");
  ...
  async function convertFiles(targets: ImageItem[], formatValue: OutputFormat, qualityValue: number) {
    ...
    try {
      for (const item of targets) {
        discard(item);
        try {
          const output = await convertGif(item.preview, item.width, item.height, formatValue, qualityValue);
  ...
  return <DashboardShell activePath="/image-tools">
    <div className="dashboard compress-pdf-page">
      {items.length ? <><input ref={inputRef} hidden multiple type="file" accept="image/gif,.gif" onChange={(event: ChangeEvent<HTMLInputElement>) => { if (event.target.files) void addFiles(event.target.files); event.target.value = ""; }} /><div className="compress-heading"><div><span className="auto-print-kicker">Free Image Tool</span><h2>GIF Converter</h2><p>Convert GIF images into JPG, PNG, WebP, BMP, ICO, PDF, or SVG.</p></div><span><ShieldCheck size={16} /> Files stay in your browser</span></div></> : null}
      {!items.length ? <><PdfToolUpload title="GIF Converter" description="Convert GIF images into JPG, PNG, WebP, BMP, ICO, PDF, or SVG - one at a time or in a batch." icon={FileImage} inputRef={inputRef} onFiles={(files) => void addFiles(files)} accept="image/gif,.gif" buttonLabel="Select GIF images" dropLabel="or drop GIF images here" headingLevel={children ? "h2" : "h1"} backHref="/image-tools" backLabel="Image Tools" />{children}</> : <div className="compress-studio">
        <section className={`compress-workspace ${dragging ? "dragging" : ""}`} onDragOver={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={drop}>
          <div className="compress-workspace-head"><div><h2>Your GIF files</h2><p>Files convert automatically after upload.</p></div><button type="button" disabled={converting} onClick={() => inputRef.current?.click()}><Plus size={18} /> Add images</button></div>
          ...
          </article>)}<button className="compress-add-card" type="button" disabled={converting} onClick={() => inputRef.current?.click()}><span><Plus size={29} /></span><strong>Add GIF files</strong><small>Click or drop more files</small></button></div>
        </section>
        <aside className="compress-side-panel"><div><span className="auto-print-kicker">RepetiGo Image Tools</span><h2>GIF Converter</h2><p>{items.length} files · {completed} converted</p></div>
          <div className="compress-side-config">
            <div className="compress-level-label"><Info size={20} /><div><strong>Output format</strong><small>Choose the file type you need GIF converted to.</small></div></div>
...
function ImagePreviewModal({ item, format, onClose }: { item: ImageItem; format: OutputFormat; onClose: () => void }) {
  ...
  return <div className="compress-preview-backdrop" role="dialog" aria-modal="true" aria-label={`Preview for ${item.file.name}`} onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className="compress-preview-modal image-compress-preview"><header><div><h2>GIF Converter preview</h2><p>{item.file.name} · {item.width} × {item.height}px</p></div><button type="button" onClick={onClose} aria-label="Close preview"><X size={21} /></button></header><div className="image-compress-compare"><article><div><strong>Original GIF</strong><span>{formatBytes(item.file.size)}</span></div><img className="compress-thumbnail-transparent" src={item.preview} alt="Original GIF" /></article><article><div><strong>Converted {format.toUpperCase()}</strong><span>{formatBytes(item.result.size)}</span></div>{canPreviewOutput ? <img className="compress-thumbnail-transparent" src={item.result.url} alt={`Converted ${format.toUpperCase()}`} /> : <div className="heic-preview-unavailable"><FileImage size={36} /><small>PDF output can't preview inline here - download it to view the file.</small></div>}</article></div><footer><div><span>Converted entirely in your browser - first frame only for animated GIFs</span></div><a href={item.result.url} download={item.result.name}><Download size={18} /> Download {format.toUpperCase()}</a></footer></section></div>;
}

function isSupported(file: File) { return file.type === "image/gif" || /\.gif$/i.test(file.name); }
...
function formatNote(format: OutputFormat) {
  if (format === "jpg") return "GIF's animation (if any) is dropped and only the first frame is used - the JPG background fills white where the GIF was transparent.";
  if (format === "png") return "PNG is lossless and preserves the GIF's transparency exactly - the safe default when you're not sure.";
  if (format === "webp") return "WebP output is smaller than PNG while still preserving transparency.";
  if (format === "bmp") return "BMP is uncompressed - files are larger than GIF, JPG, PNG, or WebP.";
  if (format === "ico") return "ICO output is best suited to small square icons and favicons, not full images.";
  if (format === "pdf") return "The GIF's first frame is embedded as a single full-page image in the PDF.";
  if (format === "svg") return "Vector tracing works best on flat-colour GIF graphics and icons - a photographic GIF traces into a large, complex file.";
  return "Converted entirely in your browser.";
}
...
async function convertGif(url: string, width: number, height: number, format: OutputFormat, quality: number) {
  if (format === "svg") {
  ...
  const context = canvas.getContext("2d"); if (!context) throw new Error("Canvas unavailable");
  if (WHITE_BACKGROUND_FORMATS.includes(format)) { context.fillStyle = "#fff"; context.fillRect(0, 0, width, height); }
  context.imageSmoothingEnabled = true; context.imageSmoothingQuality = "high";
  context.drawImage(image, 0, 0, width, height);

  if (format === "jpg") { const blob = await canvasBlob(canvas, "image/jpeg", quality / 100); return { blob, width, height }; }
