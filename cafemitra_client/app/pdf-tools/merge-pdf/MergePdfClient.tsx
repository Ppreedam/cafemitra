    ...
    <DashboardShell activePath="/pdf-tools">
      <div className={`dashboard merge-studio ${items.length ? "has-files" : "empty"}`}>
        ...
        {resultUrl ? (
          ...
          <PdfToolUpload title="Merge PDF Online Free" description="Combine PDF files in the order you want with a fast, private PDF merger." icon={Merge} inputRef={inputRef} onFiles={(files) => void addFiles(files)} headingLevel={children ? "h2" : "h1"} />
        ) : (
          <div className="merge-workbench">
            <section className={`merge-canvas ${dragging ? "dragging" : ""}`} onDragOver={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={drop}>
              <div className="merge-canvas-head"><div><h2>Arrange your PDFs</h2><p>Preview any file and remove pages you do not want.</p></div><button type="button" onClick={() => inputRef.current?.click()}><Plus size={20} /> Add PDFs</button></div>
