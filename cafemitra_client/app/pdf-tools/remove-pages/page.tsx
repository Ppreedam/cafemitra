...
export default function RemovePagesPage() {
  ...
  if (!file) return <DashboardShell activePath="/pdf-tools"><div className="dashboard remove-pages-page"><PdfToolUpload title="Remove PDF pages" description="Select and permanently remove unwanted pages from your PDF document." icon={Trash2} inputRef={inputRef} onFiles={(files) => void chooseFile(files)} multiple={false} buttonLabel="Select PDF file" /></div></DashboardShell>;
  ...
  return <DashboardShell activePath="/pdf-tools"><div className="dashboard remove-pages-page">
    ...
    </div>{error ? <div className="profile-alert error remove-error" role="alert">{error}</div> : null}
  </div></DashboardShell>;
}
