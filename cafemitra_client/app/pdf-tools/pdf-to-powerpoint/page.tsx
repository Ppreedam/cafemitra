...
const pageUrl = "https://repetigo.com/tools/pdf/pdf-to-powerpoint/";
...
1.  Run OCR on the scanned PDF first using RepetiGo's OCR PDF tool at /tools/pdf/ocr-pdf/. This adds a real text layer to the scan by recognising the characters in the image. Processing takes 10-60 seconds per page.
...
function mapSeoRoute(route: string) { const clean = route.trim().replace(/^https?:\/\/(www\.)?repetigo\.com/i, "").replace(/\/$/, ""); const routes: Record<string, string> = { "/tools/pdf": "/pdf-tools", "/tools/pdf/pdf-to-powerpoint": "/pdf-tools/pdf-to-powerpoint", "/tools/pdf/powerpoint-to-pdf": "/pdf-tools/powerpoint-to-pdf", "/tools/pdf/pdf-to-word": "/pdf-tools/pdf-to-word", "/tools/pdf/ocr-pdf": "/pdf-tools/ocr-pdf", "/tools/pdf/compress-pdf": "/pdf-tools/compress-pdf", "/tools/pdf/merge-pdf": "/pdf-tools/merge-pdf", "/products/printpilot": "/print-automation", "/features/qr-upload": "/print-automation", "/features/auto-delete": "/privacy-policy", "/security": "/privacy-policy", "/pricing": "/pricing" }; return routes[clean] || ""; }
