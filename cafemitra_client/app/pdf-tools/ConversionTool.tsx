...
async function downloadAll(results: Result[], name: string) { if (results.length === 1) { triggerDownload(results[0].url, results[0].name); return; } const zip = new JSZip(); results.forEach((result) => zip.file(result.name, result.blob)); const blob = await zip.generateAsync({ type: "blob" }); const url = URL.createObjectURL(blob); triggerDownload(url, name); setTimeout(() => URL.revokeObjectURL(url), 1500); }
