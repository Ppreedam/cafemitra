...
// Rendered at ~3x scale (216 DPI) rather than the original 2x (144 DPI) -
// edited pages are still flattened to an image (pdf-lib can't remove/edit
// existing text content streams), but the higher pixel density keeps them
// visibly sharp instead of looking noticeably lower quality than the
// untouched, still-vector pages sitting right next to them.
const EDIT_OUTPUT_SCALE = 3;
...
async function createEditedPdfImproved(file:File,pages:EditorPage[],edits:Record<string,Edit>,progress:(value:number)=>void){
  ...
  for(let number=1;number<=pages.length;number++){
    ...
    const renderPage=await renderDoc.getPage(number),viewport=renderPage.getViewport({scale:EDIT_OUTPUT_SCALE}),canvas=document.createElement("canvas");canvas.width=Math.ceil(viewport.width);canvas.height=Math.ceil(viewport.height);
...
export default function InPlacePdfEditor({ headingLevel = "h1" }: { headingLevel?: "h1" | "h2" } = {}){
  ...
  async function exportPdf(){if(!file||!Object.keys(edits).length||processing)return;setProcessing(true);setProgress(4);setError("");try{const bytes=await createEditedPdfImproved(file,pages,edits,setProgress);const blob=new Blob([bytes],{type:"application/pdf"});setResult({blob,url:URL.createObjectURL(blob),name:`${baseName(file.name)}-edited.pdf`});setProgress(100);trackToolEvent("pdf_tools","edit-pdf-inplace");}catch(reason){console.error(reason);setError("The edited PDF could not be created.");}finally{setProcessing(false);}}
  if(!file)return <><PdfToolUpload title="Edit PDF text" description="Click existing PDF wording and replace it while matching its detected font style." icon={FilePenLine} inputRef={inputRef} onFiles={files=>void chooseFile(files)} multiple={false} buttonLabel="Select PDF file" headingLevel="h2"/>{passwordModal}</>;
