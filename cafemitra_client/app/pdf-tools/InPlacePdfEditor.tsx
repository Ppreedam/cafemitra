...
export default function InPlacePdfEditor({ headingLevel = "h1" }: { headingLevel?: "h1" | "h2" } = {}){
  ...
  if(!file)return <PdfToolUpload title="Edit PDF text" description="Click existing PDF wording and replace it while matching its detected font style." icon={FilePenLine} inputRef={inputRef} onFiles={files=>void chooseFile(files)} multiple={false} buttonLabel="Select PDF file" headingLevel="h2"/>;
