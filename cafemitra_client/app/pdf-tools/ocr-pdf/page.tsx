...
export default function OcrPdfPage() {
  ...
  async function runOcr() {
    ...
    try {
      setStatus("Extracting PDF text on server...");
      const selectedIndices = Array.from(selected).sort((a, b) => a - b);
      const body = new FormData();
      body.append("pdf", file);
      body.append("pages", JSON.stringify(selectedIndices));
      const response = await fetch(apiUrl("/api/tools/extract-pdf-text/"), { method: "POST", body });
      if (!response.ok) {
        const data = await response.json().catch(() => ({}));
        throw new Error(data.message || "PDF text could not be extracted.");
      }
      setProgress(70);
      const data = await response.json();
      setStatus("Preparing PDF download...");
      const blob = await createSelectedPdf(file, selectedIndices);
...
async function createSelectedPdf(file: File, selectedIndices: number[]) { const { PDFDocument } = await import("pdf-lib"); const source = await PDFDocument.load(await file.arrayBuffer()); const output = await PDFDocument.create(); const copiedPages = await output.copyPages(source, selectedIndices.map((page) => page - 1)); copiedPages.forEach((page) => output.addPage(page)); const bytes = await output.save({ useObjectStreams: true }); return new Blob([bytes], { type: "application/pdf" }); }
