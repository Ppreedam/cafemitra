...
export default function CompressPdfPage() {
  ...
  return <DashboardShell activePath="/pdf-tools">
    <div className="dashboard compress-pdf-page">
      {items.length ? <><input ref={inputRef} hidden multiple type="file" accept="application/pdf,.pdf" onChange={(event: ChangeEvent<HTMLInputElement>) => { if (event.target.files) void addFiles(event.target.files); event.target.value = ""; }} /><div className="compress-heading"><div><span className="auto-print-kicker">Free PDF Tool</span><h1>Compress PDF</h1><p>Reduce PDF file size while balancing document quality.</p></div><span><ShieldCheck size={16} /> Files stay in your browser</span></div></> : null}

      {!items.length ? <PdfToolUpload title="Compress PDF" description="Reduce PDF file size while balancing document quality." icon={Archive} inputRef={inputRef} onFiles={(files) => void addFiles(files)} /> : <div className="compress-studio">
