...
import ResumeFormFields from "../../resume-builder/ResumeFormFields";
import { ResumePreviewPage } from "../../resume-builder/ResumePreview";
import { ResumeScaleStage } from "../../resume-builder/ResumeScaleStage";
import { useListOps } from "../../resume-builder/useListOps";
...
import { buildPreviewPdf, buildResumePdf } from "../../resume-builder/pdfBuilder";
import {
  blankResume,
  resumeHasContent,
  sampleResume,
...
import { TEMPLATES, type TemplateId } from "../../resume-builder/templates";

function customerPriceForTemplate(service: PricingService | undefined, template: TemplateId) {
  if (!service) return 0;
...
export default function PublicResumeBuilder({
  ...
  const price = customerPriceForTemplate(pricingService, resume.template);
  ...
  async function saveAndContinue() {
    ...
    setError("");
    try {
      const response = await fetch(apiUrl(`/api/public-shop/${code}/resume-builder/save/`), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ orderId: savedOrder?.id ?? null, data: resume, paymentMode }),
      });
    ...
    <article className="customer-panel pub-resume-panel is-guided">
      ...
      {isPaid ? (
        <div className="pub-resume-done">
          <p className="resbuild-customer-paid">Paid - ₹{savedOrder!.totalAmount}. Your resume is ready to download.</p>
          <ResumeScaleStage>
