// Ported from "aadhar card cutter" (aadhaar-core.js). Vector e-Aadhaar editor:
// rewrites the letter's content stream so text stays real, then crops front/back.
...
function makeAadhaarCore(PDFLib, pdfjsLib) {
  ...
  async function inspect(plainBytes) {
    ...
    let mobile = '';
    if (pdfjsLib) {
      // pdf.js v4+ destroys through the loading task (PDFDocumentProxy.destroy is gone in v6).
      const task = pdfjsLib.getDocument({ data: plainBytes.slice(0), isEvalSupported: false });
    ...
    // QR image: find the black modules so the white quiet zone baked into the
    // image can be trimmed (the card around it is white anyway).
    let qrInk = null;
    if (qrImg) {
      try {
        const xo = page.node.Resources().lookup(PDFName.of('XObject'));
        const ref = xo && xo.get(PDFName.of(qrImg.name));
        const obj = ref && doc.context.lookup(ref);
        if (obj && obj instanceof PDFRawStream) {
          const W = obj.dict.lookup(PDFName.of('Width')).asNumber();
          const Hh = obj.dict.lookup(PDFName.of('Height')).asNumber();
          const data = decodePDFRawStream(obj).decode();
          const bpp = data.length / (W * Hh);
          if (bpp === 1 || bpp === 3) { // 8-bit grey or RGB
            let x0 = W, y0 = Hh, x1 = -1, y1 = -1;
            for (let y = 0; y < Hh; y++) for (let x = 0; x < W; x++) {
              const i = (y * W + x) * bpp;
              const v = bpp === 3 ? (data[i] + data[i + 1] + data[i + 2]) / 3 : data[i];
              if (v < 128) { if (x < x0) x0 = x; if (x > x1) x1 = x; if (y < y0) y0 = y; if (y > y1) y1 = y; }
            }
            if (x1 > x0 && y1 > y0) {
              const fl = x0 / W, fr = (W - 1 - x1) / W, ft = y0 / Hh, fb = (Hh - 1 - y1) / Hh;
              // Image row 0 is drawn at the top unless the CTM flips it.
              const flipX = qrImg.ctm[0] < 0, flipY = qrImg.ctm[3] < 0;
              const w = qrImg.x1 - qrImg.x0, h = qrImg.y1 - qrImg.y0;
              const left = flipX ? fr : fl, right = flipX ? fl : fr, top = flipY ? fb : ft, bottom = flipY ? ft : fb;
              qrInk = { x0: qrImg.x0 + left * w, x1: qrImg.x1 - right * w, y0: qrImg.y0 + bottom * h, y1: qrImg.y1 - top * h };
            }
          }
        }
      } catch (e) { qrInk = null; }
    ...
    return {
      ...
      _: { frontLines, backLines, infoImg, qrImg, qrInk, bigNumBack, bigNumFront, photoImg }
    };
  }
  ...
  // Drop every BT..ET text object from page 0, leaving images and paths.
  // Used to lift the QR code cleanly: the Aadhaar number overlaps its box.
  function stripText(doc) {
    const page = doc.getPage(0);
    const content = bytesToStr(readContents(page));
    const ops = tokenize(content);
    let out = '', last = 0, bt = null;
    for (const o of ops) {
      if (o.op === 'BT') bt = o;
      else if (o.op === 'ET' && bt) { out += content.slice(last, bt.start); last = o.end; bt = null; }
    }
    out += content.slice(last);
    page.node.set(PDFName.of('Contents'), doc.context.register(doc.context.flateStream(strToBytes(out))));
  }
  ...
  return { inspect, generate, applyEdits, stripText, tokenize, CARD_W, CARD_H };
}
export default makeAadhaarCore;
